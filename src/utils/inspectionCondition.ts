...
export function getInspectionIdCandidates(car: Car | null): number[] {
    ...
    const fromPhoto = car.mainPhoto?.match(
        /\/(\d+)_\d+\.(?:jpg|jpeg|png)/i,
    )?.[1];
...
export function getDamageConditionKey(data: {
    ...
    const statusTexts = data.damages
        .flatMap((item) => item.statuses)
        .map((s) => s.toLowerCase());
    const hasReplace = statusTexts.some(
        (s) => s.includes("교환") || s.includes("교체"),
    );
    const hasRepair = statusTexts.some(
        (s) =>
            s.includes("판금") ||
            s.includes("용접") ||
            s.includes("수리") ||
