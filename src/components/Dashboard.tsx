...
function rangesEqual(a: RangeValue, b: RangeValue) {
    return a[0] === b[0] && a[1] === b[1];
}

function clampRange(range: RangeValue, limits: RangeValue): RangeValue {
    const nextMin = Math.min(Math.max(range[0], limits[0]), limits[1]);
    const nextMax = Math.max(Math.min(range[1], limits[1]), nextMin);
...
export default function Dashboard() {
    ...
    const [yearRange, setYearRange] = useState<RangeValue | null>(null);
    const [mileRange, setMileRange] = useState<RangeValue | null>(null);
    const [priceRange, setPriceRange] = useState<RangeValue | null>(null);
    const [sortKey, setSortKey] = useState<SortKey | null>(null);
    const [sortDir, setSortDir] = useState<SortDir>("asc");
    const [damageFilter, setDamageFilter] = useState<
        "all" | InspectionConditionKey
    >("all");
    const [originFilter, setOriginFilter] = useState<
        "all" | "encar" | "kbcha" | "kcar"
    >("all");
    ...
    const defaultYearRange = [limits.minYear, limits.maxYear] as RangeValue;
    ...
    const effectiveYearRange = yearRange ?? defaultYearRange;
    const effectiveMileRange = mileRange ?? defaultMileageRange;
    const effectivePriceRange = priceRange ?? defaultPriceRange;
    ...
    useEffect(() => {
        setYearRange((current) =>
            current ? clampRange(current, defaultYearRange) : null,
        );
        setMileRange((current) =>
            current ? clampRange(current, defaultMileageRange) : null,
        );
        setPriceRange((current) =>
            current ? clampRange(current, defaultPriceRange) : null,
        );
    }, [defaultYearRange, defaultMileageRange, defaultPriceRange]);
    ...
    const filtered = useMemo(() => {
        return cars.filter((car) => {
            ...
            if (q) {
                ...
                const byDigits =
                    qDigits.length > 0 &&
                    numberFields.some((value) =>
                        value.replace(/\D/g, "").includes(qDigits),
                    );
            ...
            if (
                car.year < effectiveYearRange[0] ||
                car.year > effectiveYearRange[1]
            )
                return false;
            if (
                car.mileageKm < effectiveMileRange[0] ||
                car.mileageKm > effectiveMileRange[1]
            )
                return false;
            if (
                car.priceWon < effectivePriceRange[0] ||
                car.priceWon > effectivePriceRange[1]
            )
                return false;
            if (originFilter !== "all" && car.origin !== originFilter)
                return false;
    ...
    const activeFavoriteSortedCount = useMemo(
        () =>
            sorted.filter((car) => car.isFavorite && car.isActive !== false)
                .length,
        [sorted],
    );
    ...
    const hasFilters =
        ...
        originFilter !== "all" ||
        damageFilter !== "all" ||
        !rangesEqual(effectiveYearRange, defaultYearRange) ||
        !rangesEqual(effectiveMileRange, defaultMileageRange) ||
        !rangesEqual(effectivePriceRange, defaultPriceRange);
    ...
    function scrollToCarElement(
        ...
        const target =
            matches.find((node) => node.offsetParent !== null) ??
            matches[0] ??
            null;
    ...
    function handleNavigateToCar(
        ...
        const targetCar = cars.find(
            (car) =>
                (car.origin ?? "encar") === origin && car.sourceId === sourceId,
        );
        if (targetCar) {
    ...
    function handleToggleFavorite(id: number, isFavorite: boolean) {
        ...
        toggleFavoriteMutation.mutate(
            ...
            {
                onError: (error) =>
                    setErrorToast(getDashboardErrorMessage(error)),
            },
        );
    }
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
                        ...
                        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center sm:gap-3">
                            ...
                            <button
                                onClick={handleSync}
                                disabled={
                                    !isAdmin || syncCarsMutation.isPending
                                }
                                title={
                                    isAdmin
                                        ? "Запустить обновление"
                                        : "Только администратор может обновлять"
                                }
                                aria-label={
