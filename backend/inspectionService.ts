...
function getInspectionIdCandidates(
    car: Pick<Car, "sourceId" | "url" | "mainPhoto">,
): number[] {
    const fromPhoto =
        car.mainPhoto?.match(/\/(\d+)_\d+\.(?:jpg|jpeg|png)/i)?.[1] ?? null;
...
function parsePersistedSummary(raw: unknown): InspectionSummary | null {
    ...
    if (
        ...
        typeof candidate.notFound !== "boolean"
    ) {
        return null;
    }
    return {
        ...
        replacedParts: Array.isArray(candidate.replacedParts)
            ? candidate.replacedParts
            : [],
        fetchedAt:
...
function normalizeDamageItem(item: InspectionOuterItem): InspectionDamageItem {
    ...
    const attributes = (item.attributes ?? []).filter((x): x is string =>
        Boolean(x),
    );
...
function parseInspection(
    ...
    const replacedParts = damages
        .filter((d) =>
            d.statuses.some((s) => s.includes("교환") || s.includes("교체")),
        )
        .map((d) => d.part);
...
export async function getInspectionSummaryWithCarCache(
    ...
    if (persisted) {
        let shouldPersistCar = false;
        if (!car.inspectionCondition) {
            car.inspectionCondition = getConditionKey(persisted);
            shouldPersistCar = true;
        }
        if (!car.inspectionFetchedAt) {
            car.inspectionFetchedAt = new Date();
            shouldPersistCar = true;
        }
        if (!car.inspectionCacheKey) {
            car.inspectionCacheKey = car.sourceId;
            shouldPersistCar = true;
        }
        if (shouldPersistCar) {
    ...
    const fresh = await getInspectionSummaryWithFallback(
        primaryVehicleId,
        fallbackIds,
    );
    ...
    await Promise.all(
        Array.from({ length: workerCount }, async () => {
            while (queue.length > 0) {
                ...
                const [primaryVehicleId, ...fallbackIds] =
                    getInspectionIdCandidates(car);
                if (
                    !Number.isInteger(primaryVehicleId) ||
                    primaryVehicleId <= 0
                )
                    continue;
...
function getConditionFromOpenRecord(
    payload: OpenRecordPayload,
): InspectionConditionKey | null {
    const myAccidentCost =
        typeof payload.myAccidentCost === "number" &&
        Number.isFinite(payload.myAccidentCost)
            ? payload.myAccidentCost
            : null;
    const otherAccidentCost =
        typeof payload.otherAccidentCost === "number" &&
        Number.isFinite(payload.otherAccidentCost)
...
async function getEncarOpenRecordCondition(
    vehicleId: number,
